...
const vertexAI = new src_1.VertexAI({
    project: PROJECT,
    location: LOCATION,
});
...
const generativeTextModelWithPrefixPreview = vertexAI.preview.getGenerativeModel({
    model: 'models/gemini-1.0-pro',
    generationConfig: {
        maxOutputTokens: 256,
    },
...
const generativeVisionModelWithPrefixPreview = vertexAI.preview.getGenerativeModel({
    model: 'models/gemini-1.0-pro-vision',
});
describe('generateContentStream', () => {
    ...
    it('should should return a stream and aggregated response when passed text', async () => {
        ...
        for await (const item of streamingResp.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on generateContentStream, for item ${JSON.stringify(item)}`);
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream for testing candidates in aggregated response: ${JSON.stringify(aggregatedResp)}`);
        expect(aggregatedResp.usageMetadata).toBeTruthy(`sys test failure on generateContentStream for testing usageMetadata in aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
    it('in preview should should return a stream and aggregated response when passed text', async () => {
        ...
        for await (const item of streamingResp.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview, for item ${JSON.stringify(item)}`);
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview for testing candidates in aggregated response: ${JSON.stringify(aggregatedResp)}`);
        expect(aggregatedResp.usageMetadata).toBeTruthy(`sys test failure on generateContentStream in preview for testing usageMetadata in aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
        ...
        for await (const item of streamingResp.stream) {
            ...
            for (const candidate of item.candidates) {
                for (const part of candidate.content.parts) {
                    expect(part.text).not.toContain('\ufffd', `sys test failure on generateContentStream, for item ${JSON.stringify(item)}`);
                }
            }
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream for aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
    it('in preview should not return a invalid unicode', async () => {
        ...
        for await (const item of streamingResp.stream) {
            ...
            for (const candidate of item.candidates) {
                for (const part of candidate.content.parts) {
                    expect(part.text).not.toContain('\ufffd', `sys test failure on generateContentStream in preview, for item ${JSON.stringify(item)}`);
                }
            }
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview for aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
    ...
    xit('should return a text when passed a FunctionDeclaration or FunctionResponse', async () => {
        ...
        expect((_a = aggregaratedResponse.candidates[0].content.parts[0].text) === null || _a === void 0 ? void 0 : _a.toLowerCase()).toContain(WEATHER_FORECAST, `sys test failure on generateContentStream for candidate part ${JSON.stringify(aggregaratedResponse.candidates[0].content.parts[0])}`);
    });
    ...
    xit('should return a FunctionCall when passed a FunctionDeclaration', async () => {
        ...
        for await (const item of streamingResp.stream) {
            ...
            expect(src_1.GenerateContentResponseHandler.getFunctionCallsFromCandidate((_a = item.candidates) === null || _a === void 0 ? void 0 : _a[0])).toEqual(functionCalls);
        }
    });
...
describe('generateContent', () => {
    ...
    xit('should return a text when passed a FunctionDeclaration or FunctionResponse', async () => {
        ...
        expect(resp.response.candidates[0]).toBeTruthy(`sys test failure on generateContentStream, for resp ${JSON.stringify(resp)}`);
        expect((_a = resp.response.candidates[0].content.parts[0].text) === null || _a === void 0 ? void 0 : _a.toLowerCase()).toContain(WEATHER_FORECAST, `sys test failure on generateContentStream for candidate part ${JSON.stringify(resp.response.candidates[0].content.parts[0])}`);
    });
    xit('in preview should return a text when passed a FunctionDeclaration or FunctionResponse', async () => {
        var _a;
        const request = {
    ...
    xit('should return a FunctionCall when passed a FunctionDeclaration', async () => {
        ...
        const functionCalls = resp.response
            ...
            .map((part) => part.functionCall);
        expect(functionCalls).toHaveSize(1);
        expect(src_1.GenerateContentResponseHandler.getFunctionCallsFromCandidate(resp.response.candidates[0])).toEqual(functionCalls);
    });
    xit('in preview should return a FunctionCall when passed a FunctionDeclaration', async () => {
        const request = {
            contents: [
...
describe('sendMessageStream', () => {
    ...
    it('should should return a stream and populate history when generationConfig is passed to startChat', async () => {
        ...
        for await (const item of result1.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream, for item ${JSON.stringify(item)}`);
        }
        const resp = await result1.response;
        expect(resp.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream for aggregated response: ${JSON.stringify(resp)}`);
        expect((await chat.getHistory()).length).toBe(2);
    });
    it('in preview should should return a stream and populate history when generationConfig is passed to startChat', async () => {
        ...
        const result1 = await chat.sendMessageStream(chatInput1);
        for await (const item of result1.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream in preview, for item ${JSON.stringify(item)}`);
        }
        const resp = await result1.response;
        expect(resp.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream in preview for aggregated response: ${JSON.stringify(resp)}`);
        expect((await chat.getHistory()).length).toBe(2);
    });
    ...
    xit('should return a FunctionCall or text when passed a FunctionDeclaration or FunctionResponse', async () => {
        ...
        const result1 = await chat.sendMessageStream(chatInput1);
        for await (const item of result1.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream with function calling, for item ${JSON.stringify(item)}`);
        }
        const response1 = await result1.response;
        expect(JSON.stringify(response1.candidates[0].content.parts[0].functionCall)).toContain(FUNCTION_CALL_NAME, `sys test failure on sendMessageStream with function calling, for function call: ${JSON.stringify(response1.candidates[0].content.parts[0])}`);
        expect(JSON.stringify(response1.candidates[0].content.parts[0].functionCall)).toContain('location', `sys test failure on sendMessageStream with function calling, for function call: ${JSON.stringify(response1.candidates[0].content.parts[0])}`);
        ...
        expect(response2.candidates[0].content.parts[0].text).toContain(WEATHER_FORECAST, `sys test failure on sendMessageStream with function calling, for text: ${JSON.stringify(response2.candidates[0].content.parts[0].text)}`);
    });
    xit('in preview should return a FunctionCall or text when passed a FunctionDeclaration or FunctionResponse', async () => {
        ...
        const result1 = await chat.sendMessageStream(chatInput1);
        for await (const item of result1.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream in preview with function calling, for item ${JSON.stringify(item)}`);
        }
        const response1 = await result1.response;
        expect(JSON.stringify(response1.candidates[0].content.parts[0].functionCall)).toContain(FUNCTION_CALL_NAME, `sys test failure on sendMessageStream in preview with function calling, for function call: ${JSON.stringify(response1.candidates[0].content.parts[0])}`);
        expect(JSON.stringify(response1.candidates[0].content.parts[0].functionCall)).toContain('location', `sys test failure on sendMessageStream in preview with function calling, for function call: ${JSON.stringify(response1.candidates[0].content.parts[0])}`);
        // Send a follow up message with a FunctionResponse
        const result2 = await chat.sendMessageStream(FUNCTION_RESPONSE_PART);
        for await (const item of result2.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on sendMessageStream in preview with function calling, for item ${JSON.stringify(item)}`);
        ...
        const response2 = await result2.response;
        expect(response2.candidates[0].content.parts[0].text).toContain(WEATHER_FORECAST);
    });
    xit('should return grounding metadata when passed GoogleSearchRetriever in getGenerativeModel', async () => {
        const generativeTextModel = vertexAI.getGenerativeModel({
            model: TEXT_MODEL_NAME,
...
describe('generateContentStream using models/model-id', () => {
    ...
    it('should should return a stream and aggregated response when passed text', async () => {
        ...
        for await (const item of streamingResp.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on generateContentStream using models/gemini-pro, for item ${JSON.stringify(item)}`);
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream using models/gemini-pro for aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
    it('in preview should should return a stream and aggregated response when passed text', async () => {
        const streamingResp = await generativeTextModelWithPrefixPreview.generateContentStream(TEXT_REQUEST);
        for await (const item of streamingResp.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview using models/gemini-pro, for item ${JSON.stringify(item)}`);
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview using models/gemini-pro for aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
    it('should return a stream and aggregated response when passed multipart base64 content when using models/gemini-pro-vision', async () => {
        ...
        for await (const item of streamingResp.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on generateContentStream using models/gemini-pro-vision, for item ${JSON.stringify(item)}`);
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream using models/gemini-pro-vision for aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
    it('in preview should return a stream and aggregated response when passed multipart base64 content when using models/gemini-pro-vision', async () => {
        ...
        for await (const item of streamingResp.stream) {
            expect(item.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview using models/gemini-pro-vision, for item ${JSON.stringify(item)}`);
        }
        const aggregatedResp = await streamingResp.response;
        expect(aggregatedResp.candidates[0]).toBeTruthy(`sys test failure on generateContentStream in preview using models/gemini-pro-vision for aggregated response: ${JSON.stringify(aggregatedResp)}`);
    });
});
