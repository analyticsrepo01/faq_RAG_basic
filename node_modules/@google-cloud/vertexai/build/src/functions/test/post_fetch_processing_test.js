...
const PostFetchFunctions = require("../post_fetch_processing");
...
describe('aggregateResponses', () => {
    it('grounding metadata in multiple chunks for multiple candidates, should aggregate accordingly', () => {
        const actualResult = (0, post_fetch_processing_1.aggregateResponses)(test_data_1.STREAM_RESPONSE_CHUNKS_1);
        expect(JSON.stringify(actualResult)).toEqual(JSON.stringify(test_data_1.AGGREGATED_RESPONSE_STREAM_RESPONSE_CHUNKS_1));
    });
    it('citation metadata in multiple chunks for multiple candidates, should aggregate accordingly', () => {
        const actualResult = (0, post_fetch_processing_1.aggregateResponses)(test_data_1.STREAM_RESPONSE_CHUNKS_2);
        expect(JSON.stringify(actualResult)).toEqual(JSON.stringify(test_data_1.AGGREGATED_RESPONSE_STREAM_RESPONSE_CHUNKS_2));
    });
    it('missing candidates, should return {}', () => {
        expect((0, post_fetch_processing_1.aggregateResponses)([{}, {}])).toEqual({});
    });
    it('missing role and index, should add role and index', () => {
        const actualResult = (0, post_fetch_processing_1.aggregateResponses)(test_data_1.STREAM_RESPONSE_CHUNKS_3);
        expect(JSON.stringify(actualResult)).toEqual(JSON.stringify(test_data_1.AGGREGATED_RESPONSE_STREAM_RESPONSE_CHUNKS_3));
    });
});
describe('processUnary', () => {
    it('grounding metadata in multiple candidates, processUnary should return faithful response', async () => {
        const fetchResult = new Response(JSON.stringify(test_data_1.UNARY_RESPONSE_1), fetchResponseObj);
        ...
        expect(actualResponse).toEqual(test_data_1.UNARY_RESPONSE_1);
    });
    it('response missing role and index, should add role and index', async () => {
        const fetchResult = new Response(JSON.stringify(test_data_1.UNARY_RESPONSE_MISSING_ROLE_INDEX), fetchResponseObj);
        const expectedResult = test_data_1.UNARY_RESPONSE_1;
        spyOn(global, 'fetch').and.resolveTo(fetchResult);
        const actualResult = await (0, generate_content_1.generateContent)(LOCATION, RESOURCE_PATH, TOKEN, GENERATE_CONTENT_REQUEST);
        const actualResponse = actualResult.response;
...
describe('processStream', () => {
    it('grounding metadata in multiple chunks for multiple candidates, processStream should return faithful response', async () => {
        const fetchResult = new Response(JSON.stringify(test_data_1.STREAM_RESPONSE_CHUNKS_1), fetchResponseObj);
        spyOn(global, 'fetch').and.resolveTo(fetchResult);
        ...
        expect(actualResponseToProcessStream).toEqual(test_data_1.STREAM_RESPONSE_CHUNKS_1);
    });
});
describe('processCountTokenResponse', () => {
    it('multiple contents, processCountTokenResponse should return faithful response', async () => {
        const fetchResult = new Response(JSON.stringify(test_data_1.COUNT_TOKENS_RESPONSE_1), fetchResponseObj);
        spyOn(global, 'fetch').and.resolveTo(fetchResult);
...
//# sourceMappingURL=post_fetch_processing_test.js.map